...
def free_bacon(score):
    ...
    i = 0
    while not(pi//pow(10,i) > pow(10,score) and pi//pow(10, i)<pow(10,score+1)):
        i += 1
    pi = pi // pow(10,i)
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    def turn(strategy,player_score,opponent_score,say):
        player_score,say = say_after_turn(strategy,player_score,opponent_score,say)
        if player_score > goal:
           return player_score,say
        while extra_turn(player_score,opponent_score):
            player_score,say = say_after_turn(strategy,player_score,opponent_score,say)
            if player_score > goal:
                return player_score,say
        return player_score,say

    def say_after_turn(strategy,player_score,opponent_score,say):
        player_score += take_turn(strategy(player_score,opponent_score),opponent_score,dice)  
        if who == 0:
            return player_score, say(player_score,opponent_score)
        else:
            return player_score, say(opponent_score,player_score)

    while score0 < goal and score1 < goal:
        if who == 0:
            score0,say = turn(strategy0,score0,score1,say)
        else:
            score1,say = turn(strategy1,score1,score0,say)
        who = other(who)
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    while i <= 10:
        # print(average_dice(i,dice),average_dice(high,dice))
        if average_dice(i,dice) > average_dice(high,dice):
            high = i
        i += 1
    return high
...
def bacon_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    return (free_bacon(opponent_score) < cutoff)+0 and num_rolls  # Replace this statement
    # END PROBLEM 10
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    return (free_bacon(opponent_score) < cutoff)+0 and (not(extra_turn(score+free_bacon(opponent_score),opponent_score)))+0 and num_rolls  # Replace this statement
    # END PROBLEM 11


def final_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    """Write a brief description of your final strategy.
    Implement final_strategy, which combines these ideas and any other ideas you have to achieve a high win rate against the always_roll(4) strategy. 
    Some suggestions:
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    average_dice = make_averaged(roll_dice,trials_count=100)
    cutoff = average_dice(4,six_sided) + 1 
    ...
    # while num_rolls <= 10:
    #     possiblescore = average_dice(num_rolls,six_sided)
