...
def make_generators_generator(g):
    ...
    def gen(i):
        for item in g():
            ...
            yield item
            i -= 1
    iteration = len(list(g()))
    for cnt in range(1, iteration+1):
        yield gen(cnt)
...
def deep_len(lnk):
    ...
    if lnk == Link.empty:
        return 0
    elif not isinstance(lnk, Link) or lnk.rest == Link.empty:
        return 1
    else:
...
def prune_small(t, n):
    ...
    while len(t.branches) > n:
        largest = max(t.branches, key=lambda l:l.label)
