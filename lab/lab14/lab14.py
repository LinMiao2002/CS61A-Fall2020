...
def num_splits(s, d):
    ...
    # reversed_subset = subset.copy()
    # reversed_subset.reverse()
    # for i in range(len(subset) // 2):
    #     if abs(sum(subset[i]) - sum(reversed_subset[i])) <= d :
    #         cnt += 1
    # return cnt
    cnt = 0
    l = len(s)
    def split(sub1,sub2,i):
        nonlocal cnt
        if i == l:
            if abs(sub1 - sub2) <= d:
                cnt += 1 
            return
        split(sub1 + s[i], sub2, i+1)
        split(sub1, sub2 + s[i], i+1)
    split(0,0,0)
    return cnt // 2
...
class CheckingAccount(Account):
    ...
    def deposit_check(self, check):
        if check.deposited == True or check.payable_to != self.holder:
...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        if skel_char == code_char:
            mr, mc = helper_align(skeleton_idx+1, code_idx+1)
            possibilities.append((skel_char + mr, mc))
        # Insert
        ir, ic = helper_align(skeleton_idx , code_idx + 1)
        possibilities.append(("+[" + code_char + "]" + ir, 1 + ic))
        # Delete
        dr, dc = helper_align(skeleton_idx + 1, code_idx )
        possibilities.append(("-[" + skel_char + "]" + dr, 1 + dc))
